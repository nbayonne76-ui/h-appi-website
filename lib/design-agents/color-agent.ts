// Color Agent — specialized agent generating color schemes from brand context.
// Ported 1:1 from microsoft-sales-app's lib/design-agents/color-agent.js.

import type { ColorAgentResult, DesignBrief } from './types';
...
export async function generateColorSchemes(
  brief: Pick<DesignBrief, 'brandContext' | 'targetAudience' | 'industry' | 'mood'>
): Promise<ColorAgentResult> {
  const { industry, mood, targetAudience } = brief;
  const basePalette = INDUSTRY_PALETTES[industry] || INDUSTRY_PALETTES.default;

  const schemes: ColorAgentResult['schemes'] = [
    {
      id: 'scheme_1',
      name: 'Primary Bold',
    ...
    },
    {
      ...
      colors: {
        primary: lightenColor(basePalette[0], 20),
        secondary: lightenColor(basePalette[1], 15),
        accent: lightenColor(basePalette[2], 25),
        background: '#0F172A',
        surface: '#1E293B',
        text: '#F1F5F9',
