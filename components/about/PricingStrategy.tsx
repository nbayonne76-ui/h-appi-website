          ...
          <div className="grid md:grid-cols-2 gap-3">
            {Array.from({ length: 4 }).map((_, i) => (
              <div key={i} className="flex items-start space-x-2">
                ...
                <span className="text-sm text-gray-700">{t(`problem.items.${i}`)}</span>
              </div>
            ))}
          </div>
          ...
          <table className="w-full border-collapse">
            <thead>
              <tr className="border-b-2 border-happi-blue/20">
                <th className="text-left py-4 px-4 text-happi-dark">{t('comparison.colType')}</th>
                <th className="text-center py-4 px-4 text-happi-green font-bold">{t('comparison.colSavings')}</th>
                <th className="text-left py-4 px-4 text-happi-dark">{t('comparison.colBenefit')}</th>
              </tr>
            </thead>
            <tbody>
              {Array.from({ length: 5 }).map((_, i) => (
                <tr key={i} className="border-b border-gray-100 hover:bg-happi-gray/50">
                  <td className="py-4 px-4 text-happi-dark font-medium">{t(`comparison.rows.${i}.type`)}</td>
                  ...
                  <td className="py-4 px-4 text-gray-600">{t(`comparison.rows.${i}.benefit`)}</td>
                </tr>
              ))}
            </tbody>
          </table>
